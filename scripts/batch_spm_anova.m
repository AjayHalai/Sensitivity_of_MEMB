...
% Updated 9-11-18 so user can specify whether each user_reg is a confound or covariate of interest (affecting nonsphericity) - !!!have not tested for user_regs that separate for each group
...
try 
    user_reg_conf = S.user_reg_conf;
catch
    user_reg_conf = ones(1,size(user_regs,2)); % Assume all confounds
end
...
if iscell(user_regs)
    for g = 1:ngrp
        iH = [iH [[1:ncon] find(~user_reg_conf) + ncon] + (ncon+nreg)*(g-1)]; % Columns of interest
        iC = [iC            find(user_reg_conf) + ncon + (ncon+nreg)*(g-1)]; % Confounds
    end
else
    iH = [[1:nH] find(~user_reg_conf)+nH];
    iC = [(length(iH)+1):size(X,2)];
end
...
SPM.xX = struct(...
        'X',X,...
        'iH',iH,'iC',iC,'iB',[(nR+1):size(X,2)],'iG',zeros(1,0),...
        'name',{cname},'I',Ind,...
