...
for s=1:size(subcode,2)
    ...
    for c=1:size(cond,1)
    cd(root)
    outdir=([root,'/GLM/MVPA_first/',subcode{s},'/',cond{c},'odd/'])
    ...
    delete([outdir,'/SPM.mat']);
        ...
        if c<=1
            gunzip(['./fmriprep/',subcode{s},'/func/',subcode{s},'_task-',cond{c},'_*_bold.nii.gz'],[datadir,'/']);
            system(['fslmaths ',datadir,'/sub* -mas ./fmriprep/',subcode{s},'/anat/*MNI*mask.nii.gz ',datadir,'/',subcode{s},'_bold -odt input']);
            system(['fslselectvols -i ',datadir,'/',subcode{s},'_bold -o ',datadir,'/tmp_bold.nii --vols=tmp.txt']);
            ...
            system(['fslselectvols -i ',datadir,'/sub* -o ',datadir,'/tmp_bold.nii --vols=',datadir,'/tmp.txt']);
    ...
    if c == 2
        cd(root) 
        outdir=([root,'/GLM/MVPA_first/',subcode{s},'/',cond{c},'odddn/'])
        datadir=([root,'SPM/',subcode{s}])
        mkdir(outdir);
        mkdir(datadir);
        delete([outdir,'/SPM.mat']);
